...
def update(rType="MAIN"):
    if rType == "UPDATE":
        rlink = "http://ariva150.altervista.org/index1.php?dir=Xtreamui/&file=update.zip"
    else:
...
if __name__ == "__main__":
    ...
    if not rVersion in rVersions:
        printc("It can only be installed on Ubuntu 22.04")
        sys.exit(1)
    printc("X-UI installer in Ubuntu %s - Azzosa-UIMod" % rVersion, col.GREEN, 2)
    print(" ")
